...
Astra's whole-system review of candidate `634635388837d39e1f1c9f33e2a20d8e2121c81d` found real cutover blockers rather than missing Product scope. Its follow-up review of remediation candidate `7dff1bc34929eb31961d222491eb5ba1a9ba8d5e` found a second bounded set of implementation blockers around nested Git metadata, concurrent path substitution, journal placement/order, writable scratch exhaustion, runtime-capture races, and diagnostic/lifecycle identity.

Astra's next independent review of candidate `37bd705ae3f7b383f2b46eb1f75cb4456b20c472` returned `RECONCILE BEFORE ACCEPTANCE` with exactly three remaining blockers: F1, concurrent parent substitution could redirect the filesystem helper into protected in-root authority paths; F2, `RENAME_EXCHANGE` was still being treated like a CAS failure after an effect and could trigger destructive reverse-exchange rollback or an invalid `precondition_failed`; and F3, PostgreSQL server-side writes could escape the intended worker scratch limits through host-writable socket/storage surfaces. Astra explicitly found no need for MCP-05, new architecture, or a new tool surface.
...
- pinned manifest reads plus descriptor-relative parent traversal using `O_DIRECTORY | O_NOFOLLOW`, with mutation effects performed relative to pinned parent directory descriptors so concurrent parent substitution cannot redirect the helper into another in-root authority path;
- conditional create/replace/delete semantics that return `precondition_failed` only before an effect, never attempt destructive reverse-exchange rollback after `RENAME_EXCHANGE`, and preserve deterministic operation-scoped recovery bytes whenever a post-effect result is uncertain;
...
- disposable PostgreSQL 18 with no network, read-only root, `no-new-privileges`, fixed UID/GID, fixed memory/swap/PID ceilings, bounded tmpfs server storage, and an exported Unix-socket directory created only on verified host `/dev/shm` tmpfs; server-side `COPY` and root-write probes verify the storage contract while PostgreSQL is still running;
...
- The tracer ran only in an automatically removed clone with a temporary new-MCP manifest/operator policy and a copied provisioned DD runtime whose editable-install path was retargeted to the disposable clone before sanitization. Live Discrepancy Desk remained exact `73941507f2511aca3af8abcf47799bd016c687bf`, `main`, and clean afterward.
